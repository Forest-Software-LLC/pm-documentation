...
(If your `forest.json` has a `root` field, meaning you're authoring a package rather than a game, the `Packages/` folder lives next to the root module instead, e.g. `src/Packages/`. See [Package Anatomy](/platforms/roblox/anatomy#developing-with-dependencies).)
