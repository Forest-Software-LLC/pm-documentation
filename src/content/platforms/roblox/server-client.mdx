...
Recall [the entry point rule](/platforms/roblox/anatomy#the-entry-point-rule): a package has exactly one root module, and whatever it returns is the whole public API. Forest also has **no realm concept**, so there are no separate server-only/client-only install targets ([details](/platforms/roblox/installing#where-packages-live)). Both facts point at the same design:
