...
Only for **[private packages](/concepts/private-packages) owned by the Studio**. Public packages are accessible to everyone by definition, and admins/owners always have access to all of the Studio's packages. Permissions exist to selectively open private packages to members.
...
**Studio API tokens** use the same system: a token set to **Selected packages** reads what its roles and direct grants allow, like a member. Token grants are always `read`. See [CI & API Tokens](/features/ci).
...
- Install it with `forest i studio/package` (the registry issues them the short-lived signed download URLs; see [Private Packages](/concepts/private-packages)).
