...
- **Authentication Data**: session tokens (refresh tokens are stored only as hashes) and identifiers for third-party accounts you link (e.g., your Discord user ID). OAuth access tokens from linked accounts are used only during the linking flow and are not retained.
- **Usage Data**: aggregate usage metrics, package metadata (names, descriptions, README files), and logs of CLI/Service activity.
- **Organization Data**: private audit logs tied to your organization’s account.
- **Payment Information**: billing details processed securely by third-party payment providers (e.g., Stripe). We do not store full credit card numbers.
...
- Improve discovery and recommendations (e.g., AI analysis of public package content such as READMEs and license text; private package content is never sent to AI providers).
...
- **Vendors/Service Providers**: such as hosting and infrastructure (e.g., Cloudflare, MongoDB Atlas), payment processors (e.g., Stripe), email delivery (e.g., Postmark), and AI providers (e.g., OpenAI, which processes public package content only, never private package content).
...
- Encryption at rest for sensitive data such as two-factor authentication secrets.
- Industry-standard password hashing for account credentials.
- Session refresh tokens are stored only as hashes; OAuth access tokens from linked accounts are not retained.
...
## Children’s Privacy
The Services are not directed to children under 13, and we do not knowingly collect personal information from them. Users between 13 and 18 must have parental or guardian consent.
